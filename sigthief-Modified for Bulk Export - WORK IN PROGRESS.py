...
def truncate(exe, output):
    ...
    if flItms['CertLOC'] == 0 or flItms['CertSize'] == 0:
        # not signed
        print("Input file " + exe + " Not signed!")
        sys.exit(-1)
    else:
        print( "Inputfile is signed!")
...
def ripall():
    ...
    for filename in os.listdir(path):
        ...
        if os.path.isfile(f) and (f.endswith(".exe") or f.endswith(".msi") or f.endswith(".dll")) :
            output = str(f) + ".cer"
            cert = copyCert(f)
            print("Output file: {0}".format(output))
            fo = open(output, 'wb')
...
if __name__ == "__main__":
    ...
    parser.add_option('-f', '--all',dest="ripall", action='store_true',
                  help='Rip All Certificates from Folder')
